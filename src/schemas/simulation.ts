...
export const ObjectConfigSchema = z.preprocess(
  ...
  y: z.number().describe('Initial Y position of the object\'s center, in configured units. With default settings, canvas is 60m tall. Y=0 is bottom edge, Y increases upward. For container objects with mode "grounded" (the default), ramp objects (always ground-seated), and objects carrying `seatOn`, y is derived at load — author 0 as a placeholder.'),
...
export const SliderConfigSchema = z.object({
  ...
  property: z.string().describe('Property path to control. Cartesian components: "velocity.x", "velocity.y", "position.x", "position.y", "acceleration.x", "acceleration.y". Polar projections: "velocity.magnitude" / "velocity.angle" (and the same for acceleration). A ".magnitude" slider changes speed while preserving direction; an ".angle" slider rotates the vector while preserving magnitude — ideal for projectile launch (a Speed slider + a Launch-angle slider). Angle is in the environment\'s angleUnit (default degrees), measured counter-clockwise from +X. Scalars: "mass", "restitution", "friction", "angle", "isStatic". Ramp-dimension paths (target must be a `ramp` object): "ramp.angle", "ramp.rise", "ramp.run", "ramp.slopeLength" — the slider overrides that synthesis param and the incline rebuilds live (seated riders re-seat automatically); the overridden param\'s companion is the first-authored remaining param, so author {angle, slopeLength} for a tilt-a-fixed-board angle slider (the classic tilt-until-slip friction demo). Note: an "acceleration.*" slider drives an ADDITIONAL constant acceleration on top of environment gravity (rocket thrust, braking, conveyor push) — not a gravity override.'),
