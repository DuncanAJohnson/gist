...
function AuthoringJson() {
  ...
  { "kind": "velocity", "components": true }    // dashed vₓ / v_y legs
      ...
      <FieldTable
        rows={[
          { name: 'kind', type: "VectorKind", req: true, desc: <>"velocity", "acceleration", "force-net", "force-gravity", "force-drag" render today (drag is non-zero only in air-resistance mode). "force-applied" / "force-friction" exist in the enum but aren&rsquo;t wired yet (they await the engine contact-force seam) — don&rsquo;t author those two. Note: the LLM prompt still discourages authoring force-gravity/-drag on generated sims (debug-first) — but hand-authored local sims may use them freely.</> },
