...
- ⚠️ **DEPENDENCY (named 2026-08-09): G6-with-friction is BLOCKED on the friction-representation decision.** The deficit claim above is *quantitative* — the student checks ΔE against µ·N·d. Both engines combine contact friction as **Max of the pair**, so if any other body carries a higher µ, the operative contact µ is not the µ the student sees and the arithmetic fails. The `friction-slider-masked` diagnostic surfaces the case but does not fix it; the decision (pair-friction authoring vs. a one-owner convention enforced at the seam) is HELD — see `Notes_on_Ramps_and_Tracks_Refactor.md` → Open questions. **The frictionless half of this graph carries the subtler version of the same dependency:** "E_total drift is numerical artifact by definition" only holds if the scene is *certainly* frictionless, and under Max masking authoring µ = 0 on the mover doesn't guarantee that — so drift and unintended friction become indistinguishable. Resolve alongside FBD step 5 (an analytical friction model needs a defensible contact µ for the same reason).
- **Energy is DERIVED, not summed** — the same rule as invariant #14 (net force is derived, never summed). Compute E_total(t) from sampled state; the friction loss is the *residual*. Do NOT integrate ∫F_fric·v dt to construct it — the analytical µ·N·d prediction is the *overlay you compare against*, which makes the closure a validation rather than a construction. Consistent with harness rule 4 below.
