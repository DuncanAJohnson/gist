...
3. **Ramps / inclined planes.** When a skeleton entry carries `"ramp": true`, emit a `ramp` object on that ObjectConfig INSTEAD OF `width`/`height`/`svg` — the runtime synthesizes the right-triangle sprite and collider and seats the ramp on the floor:
   ...
   - **Seating an object on the incline:** give that object `"seatOn": "<ramp id>"` (the same field as `"seatOn": "ground"` above, pointed at a ramp instead of the floor) and author only its `x` (with `y: 0` as a placeholder, no `angle`) — the runtime derives y and angle so it rests flush on the surface. `seatOn` is a START POSE, not an attachment: once the sim runs the object slides, rolls, or leaves the ramp under normal physics. This is THE way to author "a block released on an incline" or "a ball rolls down a ramp". A seated rider at rest aims down-slope automatically, so a `"velocity.magnitude"` slider gives an "initial speed down the incline" control with no extra authoring.
   - **RESTING ON THE FLOOR: author `"seatOn": "ground"` with `"y": 0`.** `y` is the object's CENTER, so `"y": 0` on its own puts an object HALF BELOW THE FLOOR, and guessing `height/2` leaves it floating or sunk depending on the sprite. `"seatOn": "ground"` is the only way that is always right — the runtime derives the resting y (and accounts for an authored `angle`, so a tilted box rests on its corner). Use it for anything that starts at rest on the ground: a crate, a sled, a cart, a box. Requires `environment.walls` to include `"bottom"`. This is the flat-ground twin of the ramp form below.
4. Set physics fields appropriate to the object's role: `velocity`, `acceleration` (rare — gravity is usually enough, and see the chapter rule below), `appliedForce` (the DYNAMICS counterpart — see the same rule), `mass`, `restitution`, `friction`, `isStatic`, `angle`, `angularVelocity`, etc.
   - **`acceleration` is a KINEMATICS-chapter field.** It stipulates "this body accelerates at a" and adds that acceleration on top of gravity every frame. Its cause is deliberately unmodelled — no force is behind it — which is exactly right for the chapter where constant acceleration is taught BEFORE Newton's laws (a cart speeding up, a rocket's constant thrust, steady braking, a conveyor push). Author it when the prompt describes MOTION ("speeds up at 2 m/s²", "decelerates steadily") rather than a CAUSE.
   - **Do NOT reach for it when the prompt describes a push, pull, or force** ("push the crate with 50 N", "how hard do you have to shove it"). Those are DYNAMICS scenes and belong to Newton's second law. Mixing the two on one body is the trap: the stipulated acceleration still moves the body and still shows up in `"force-net"` as m·a, but no force arrow explains it, so a free-body diagram will not close and the sim looks broken to a student who is adding up forces. Pick the chapter the prompt is in and stay in it. Use the role from the skeleton to decide — e.g. a "platform" should be `isStatic: true` (inclines are authored via the `ramp` field above, never as a manifest-svg triangle); a "projectile" needs an initial velocity; a "ball" gets restitution ~0.8.
   - **`appliedForce` is the DYNAMICS-chapter field — the one to use for a push or a pull.** It is a constant force in NEWTONS on that body: `"appliedForce": {"x": 50, "y": 0}` or, in polar, `{"magnitude": 50, "angle": 30}`. Unlike `acceleration` it names the CAUSE, so `a = F/m` emerges from the physics: a mass slider next to it genuinely changes the acceleration, and a free-body diagram CLOSES. Reach for it whenever the prompt says push, pull, shove, tow, thrust, drag-it-along, or gives a force in newtons. It superposes honestly with gravity, friction, drag and contacts — below `µ·m·g` on a body with friction the object correctly does NOT move, which is the whole "you have to push hard enough" lesson.
     - It acts at the body's CENTER OF MASS in WORLD coordinates: it never tips or spins a body, and its direction does not rotate with the body. So it models a steady push or a wind, not a body-fixed rocket thruster (for thrust in a rotating frame, there is no field yet — don't fake it).
     - It is CONSTANT for the whole run — there is no duration, schedule, or "push for 2 seconds" yet. If the prompt wants the force to vary, give the student a slider (see FILL CONTROLS) rather than inventing a time-varying field.
     - **Never author both `acceleration` and `appliedForce` on the same body.** They superpose (`a_total = a_authored + F/m`), so the body outruns what `F/m` predicts and the mass slider stops telling the truth. Pick the chapter.
   - **Vector fields can be authored two ways** — components `{"x": ..., "y": ...}` or polar `{"magnitude": ..., "angle": ...}` (magnitude ≥ 0, in the field's own units — units/second for `velocity`, units/s² for `acceleration`, newtons for `appliedForce`; angle in the environment `angleUnit`, default degrees, counter-clockwise from +X). This applies to `velocity`, `acceleration`, and `appliedForce` alike. They are equivalent; pick whichever reads more naturally. **Prefer polar when the prompt speaks in magnitude-and-direction terms** — "launched at 20 m/s at 45°" becomes `"velocity": {"magnitude": 20, "angle": 45}`, and "pull the sled with 50 N at 30° above horizontal" becomes `"appliedForce": {"magnitude": 50, "angle": 30}`, both with zero trigonometry. Use components when the prompt is axis-aligned ("moving right at 5 m/s", "push it horizontally with 20 N"). NEVER mix fields from both forms in one value (no `{"x": ..., "angle": ...}`).
...
6. **Vector arrows on objects** (`showVectors`, optional): an array declaring which physics vectors to draw on the body as it moves. Each entry is either a shorthand kind string for default styling, or a full `{kind, color?, label?, labelPlacement?, labelFontSize?, pixelsPerUnit?}` config for per-arrow overrides. Available kinds:
   ...
   - `"force-net"` (red) — the net force, computed as `m · a_derived`. Use for Newton's 2nd Law dioramas, collision impulses, or whenever ΣF is the pedagogical focus. It is DERIVED from the body's measured motion, never summed from the component arrows below — which is what makes the component arrows a check on the physics rather than a restatement of it.
   - The COMPONENT force kinds, all available: `"force-applied"` (the `appliedForce` you authored), `"force-friction"`, `"force-normal"`, `"force-drag"` (air resistance), `"force-gravity"`. These are the pieces of a free-body diagram. Author the ones the lesson actually names — a crate being pushed across a floor wants `["force-applied", "force-friction", "force-net"]`; a falling object with air resistance wants `["force-gravity", "force-drag", "force-net"]`.
   - Combine kinds freely on one body. Examples: a projectile that should show both motion and acceleration → `"showVectors": ["velocity", "acceleration"]`. A free-fall apple → `"showVectors": ["velocity", "acceleration"]` (acceleration stays constant, velocity grows). A bowling ball whose net force matters → `"showVectors": ["force-net"]`. A cart in a Newton's-2nd-Law setup → `"showVectors": ["force-applied", "force-friction", "force-net"]`.
   - **A free-body diagram should CLOSE:** the component arrows you draw ought to add up to `"force-net"`. If a body carries `acceleration` (the forceless kinematics field) they will not close, because no arrow explains that motion — one more reason not to mix the chapters.
...
- `property` MUST be a scalar dot-path — always include the axis suffix on vector quantities. Valid Cartesian: `"velocity.x"`, `"velocity.y"`, `"position.x"`, `"position.y"`, `"acceleration.x"`, `"acceleration.y"`, `"appliedForce.x"`, `"appliedForce.y"`, `"mass"`, `"restitution"`, `"friction"`, `"angle"`, `"isStatic"`. Valid POLAR projections: `"velocity.magnitude"` / `"velocity.angle"` (and the same for `acceleration` and `appliedForce`). A `.magnitude` slider changes speed while preserving direction; an `.angle` slider rotates the vector while preserving magnitude. A `.magnitude` slider's `min` must be ≥ 0 (magnitude is a length — negative values clamp to 0 at runtime). Invalid: `"velocity"`, `"position"`, `"acceleration"` alone (these are 2D vectors and a slider can only drive one number at a time). The `targetObj` must equal the skeleton's `target_id`.
...
- **Acceleration sliders are additional-acceleration knobs, NOT gravity overrides.** Environment gravity always acts on every non-static body via the physics engine; a slider bound to `"acceleration.x"` or `"acceleration.y"` adds a *second* constant acceleration on top. Use them when the lesson involves a non-gravity constant force: rocket thrust (`"acceleration.y"`, default ~15 m/s² upward), braking deceleration on a moving body (`"acceleration.x"` with sign opposite to velocity), constant horizontal wind, magnetic-field push on a uniform-field body. Do NOT use an `"acceleration.y"` slider as a way to set or change gravity — that would double-count. For free-fall and projectile sims, leave acceleration unset (gravity alone) and let students see the kinematics emerge from gravity. **And do not use an acceleration slider as a stand-in for a force slider** — an acceleration slider deliberately ignores mass (that is what makes it a kinematics tool), so moving a mass slider next to it changes nothing, which is the exact opposite of the a = F/m lesson. Keep acceleration sliders in kinematics scenes; see the chapter rule in FILL OBJECTS.
- **Force sliders (`"appliedForce.x"`, `"appliedForce.y"`, `"appliedForce.magnitude"`, `"appliedForce.angle"`) are THE control for a Newton's-2nd-Law sim** — this is the knob PhET's *Forces and Motion* is built around. Label it in newtons ("Applied force (N)", "Push strength (N)"). The lesson gets much stronger when you pair it with a `"mass"` slider: same force, heavier body, smaller acceleration — `a = F/m` from both sides, which an acceleration slider can NEVER teach because it ignores mass by design. On a body with friction, choose a range that straddles `µ·m·g` so students can find the breakaway point themselves (e.g. µ = 0.4, m = 5 kg, g = 9.8 → breakaway ≈ 19.6 N, so a 0–40 N slider). An `"appliedForce.angle"` slider paired with a magnitude slider is the classic "what angle makes the sled easiest to pull?" investigation — the optimal angle is `arctan(µ)`, and pulling upward at an angle reduces the normal force and therefore the friction.
- **The slider's `targetObj` must be the object that CARRIES the force — the one being pushed, not the one doing the pushing.** A slider on a named object changes the forces in THAT object's free-body diagram, and GIST draws one body's diagram at a time: every force you author is a force ON that body. So in a "person pulls a sled" scene the force and BOTH its sliders target the **sled** — the body that accelerates — and the person is scenery, a visual label for where the pull comes from, carrying no physics. Do not author a reaction force on the person: force PAIRS are out of scope, and the sled's own diagram is the whole lesson. Binding a force slider to the puller is a silent no-op — the force never reaches the physics, and if the puller is `isStatic` nothing happens at all. Same rule for readouts and graphs.
- The object being controlled must already declare the field: an `"appliedForce.x"` slider with `defaultValue: 20` needs that object to carry `"appliedForce": {"x": 20, "y": 0}` in FILL OBJECTS.
- The `"force-*"` paths (`"force-net.*"`, `"force-friction.*"`, `"force-normal.*"`, `"force-drag.*"`, `"force-gravity.*"`) are **READ-ONLY** — they are measured from the physics, not set. They belong in outputs and graphs; a force slider always binds to `"appliedForce.*"`, the one force with a writable source.
- Choose realistic ranges: velocities -30 to 30, masses 0.1 to 100, non-gravity acceleration -20 to 20 m/s², applied force -100 to 100 N (and see the breakaway-straddling rule above when the body has friction). The `defaultValue` should match the object's current state from the objects stage (so an `"acceleration.y"` slider with default 15 needs the same object to declare `"acceleration": {"x": 0, "y": 15}` in FILL OBJECTS).
...
- `property` MUST resolve to a scalar number — always include the axis suffix on vector quantities (e.g. `"velocity.y"`, NOT `"velocity"`). To plot a 2D quantity, emit two separate lines. Polar projections work too: `"velocity.magnitude"` (speed) and `"velocity.angle"` (direction, in the environment `angleUnit`), and the same for `acceleration` and `appliedForce`. Overlaying `["velocity.x", "velocity.y", "velocity.magnitude"]` is a useful components-vs-resultant view; overlaying `"appliedForce.x"` with `"acceleration.x"` shows `a = F/m` directly — the two traces are proportional, and the constant of proportionality is 1/mass.
- **Forces are plottable too** (read-only, newtons): every vector-arrow kind — `"force-net.*"`, `"force-friction.*"`, `"force-normal.*"`, `"force-drag.*"`, `"force-gravity.*"`, `"force-applied.*"` — with `.x`, `.y`, `.magnitude` or `.angle`. Two high-value pairings: `"force-applied.x"` against `"force-friction.x"` is the "why doesn't it move yet?" graph (the traces cancel below breakaway and separate above it), and `"force-gravity.y"` against `"force-drag.y"` is the terminal-velocity graph (they converge to equal-and-opposite exactly as `velocity.y` flattens).
...
- ✅ Cartesian: `"velocity.x"`, `"velocity.y"`, `"position.x"`, `"position.y"`, `"acceleration.x"`, `"acceleration.y"`, `"appliedForce.x"`, `"appliedForce.y"`
- ✅ Polar projections: `"velocity.magnitude"` (speed, |v|), `"velocity.angle"` (direction), and the same for `acceleration` and `appliedForce`. Use `"velocity.magnitude"` directly for a speed readout — set its `unit` to "m/s". Angle is in the environment `angleUnit` (default degrees), counter-clockwise from +X — set its `unit` to "°" (or "rad"/"rev" to match).
- ✅ `"appliedForce.magnitude"` is the readout for "how hard am I pushing?" — set its `unit` to "N". It reads the force actually acting on the body, the same number the `"force-applied"` arrow draws.
- ✅ **FORCES — every vector-arrow kind is also a readable path** (units "N"): `"force-net.*"`, `"force-friction.*"`, `"force-normal.*"`, `"force-drag.*"`, `"force-gravity.*"`, `"force-applied.*"`, each with `.x`, `.y`, `.magnitude` or `.angle`. A readout returns exactly the number its arrow draws. `"force-net.magnitude"` is ΣF measured from the body's actual motion (m·a), so a group listing the components alongside it is **the numeric twin of a free-body diagram** — and if the components don't sum to the net, that is the physics telling you something is unaccounted for. These are READ-ONLY (see FILL CONTROLS).
- ✅ scalars: `"mass"`, `"angle"`, `"angularVelocity"`, `"restitution"`
- ❌ NEVER `"velocity"`, `"position"`, `"acceleration"`, or `"appliedForce"` alone — these are 2D vectors and the UI will crash trying to render them. (To show speed, use `"velocity.magnitude"`, NOT two separate axis readouts.) Same for a bare `"force-net"` — always add the suffix.
...
- "objects" — the physics objects array. Re-run if the edit adds/removes/moves an object, changes its size or svg, or changes its physics (mass, velocity, acceleration, appliedForce, restitution, isStatic, friction, angularVelocity, etc.). ALSO re-run for any change to a container's proportions/walls/mode, to a ramp's angle/size/direction ("make the ramp steeper", "flip the incline", "longer slope"), or to where an object sits on a ramp — `container`, `ramp`, and `seatOn` all live on the object. ALSO re-run for any change to the vector arrows drawn ON a body — the `showVectors` field lives on the object, so requests like "show the velocity arrow", "add net-force arrows", "show the velocity components", "hide the acceleration arrow", or "also show the full / whole / resultant velocity vector" are OBJECTS edits (NOT graphs or outputs, even though velocity can also be plotted there).
